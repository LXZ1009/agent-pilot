...
export function parseWorkbenchCommand(input: string): WorkbenchCommand {
  ...
  if (!raw) {
    return command('unknown', raw, null, null, [], '');
  }
  ...
  if (slash === '/prepare') {
    return command('prepare', raw, pickMeetingId(raw), null, targets, body);
  }

  if (slash === '/material') {
    return command('material', raw, null, null, targets, body || '生成会议物料');
  }

  if (slash === '/preview') {
    return command('preview', raw, null, null, targets, body || '生成会前预览');
  }

  if (slash === '/trace') {
    return command('trace', raw, null, null, targets, body || 'last');
  }

  if (slash === '/retry') {
    return command('retry', raw, null, firstTarget, targets, body);
  }
...
function displayAgentName(agent: string): string {
  if (agent === 'pre_meeting_interview_agent') return 'PreMeetingInterviewAgent';
  if (agent === 'interview_structuring_agent') return 'InterviewStructuringAgent';
  if (agent === 'material_asset_agent') return 'MaterialAssetAgent';
  if (agent === 'notification_agent') return 'NotificationAgent';
  if (agent === 'task_tracking_agent') return 'TaskTrackingAgent';
  if (agent === 'pricing_meeting_agent') return 'PricingMeetingAgent';
  return agent;
}
